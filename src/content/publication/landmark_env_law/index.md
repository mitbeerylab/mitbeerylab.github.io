---
title: "A landmark environmental law looks ahead"
publishDate: "21 December 2023"
venue: Science
external: https://arxiv.org/abs/2406.11608
thumbnail: ./images/sara17.jpg
note: "PHOTO: ANTHONY SOUFFLE/STAR TRIBUNE/GETTYIMAGES"
authors:
  - name: Robert L Fischman
  - name: JB Ruhl
  - name: Brenna R Forester
  - name: Tanya M Lama
  - name: Marty Kardos
  - name: Grethel Aguilar Rojas
  - name: Nicholas A Robinson
  - name: Patrick D Shirey
  - name: Gary A Lamberti
  - name: Amy W Ando
  - name: Stephen Palumbi
  - name: Michael Wara
  - name: Mark W Schwartz
  - name: Matthew A Williamson
  - name: Tanya Berger-Wolf
  - ref: "~sara"
  - name: David Rolnick
  - name: Justin Kitzes
  - name: David Thau
  - name: Devis Tuia
  - name: Daniel Rubenstein
  - name: Caleb R Hickman
  - name: Julie Thorstenson
  - name: Gregory E Kaebnick
  - name: James P Collins
  - name: Athmeya Jayaram
  - name: Thomas Deleuil
  - name: Ying Zhao
annotationLegend:
  "*": "Equal contribution"
  "†": "Equal advising"
---
